...
print_hex(Packet) when is_binary(Packet) -> io_lib:format("HEX dump:~n",[]) ++ print_hex(Packet, 0).

print_hex(Packet, Line) when is_binary(Packet) andalso size(Packet) =< ?BYTES_PER_LINE->
  L1 = print_line_number(Line), 
  L2 = case Packet of 
    <<L:(?BYTES_PER_LINE div 2)/binary, R/binary>> ->
      io_lib:format("~s    ", [binary_to_hexstring(L)]) ++
      io_lib:format("~s~n", [binary_to_hexstring(R)]);
    Packet -> io_lib:format("~s~n", [binary_to_hexstring(Packet)])
  end,
  L1++L2;
...
print_eth(<<Dst:6/bytes, Src:6/bytes, Type:16/integer, _Packet/binary>>) ->
  io_lib:format("ETH: ===> DST: ~s | SRC: ~s| Type: ~p ", [binary_to_hexstring(Dst), binary_to_hexstring(Src), l3(Type)]);
print_eth(_P) -> ok.
...
print_ipv4(<<_MAC:12/bytes, 16#0800:16, Packet/binary>>) ->
 <<_V:4, _HLen:4, _SrvcType:8, _TotLen:16, _ID:16, _Flgs:3, _FragOff:13, _TTL:8, Proto:8, _HdrChkSum:16, 
  ...
  io_lib:format("IPv4: ===> SRC: ~p.~p.~p.~p | DST: ~p.~p.~p.~p | Type: ~p ", 
